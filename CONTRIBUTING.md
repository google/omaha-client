...
We'd love to accept your patches and contributions to omaha-client. There are
...
guide its design. Since omaha-client is a library, this advice is not as
applicable; we want our API surface to be featureful and complete even if not
every feature or method has a known use case. However, the observation that
unused code is hard to design still holds.
...
Some of the code in omaha-client has the property that, if it is buggy, those
bugs may not cause user code to fail. This makes it extra important to write
thorough tests, but it also makes it harder to write those tests correctly. Here
are some guidelines on how to test code in omaha-client:
1. All code added to omaha-client must include tests that exercise it
   completely.
...
name of that subsystem in square brackets, omitting any "omaha-client" prefix
(that's implicit). For example, for a commit adding a feature to the protocol
leveraging a new server API:

```text
[protocol] Support server side API for <feature>
```
...
1. Non-public URLs
1. Individuals
1. Hosted code reviews (such as on https://github.com/google/omaha-client/pulls)
    ...
    + Refer to commits in other repositories by public web address (such as
      https://github.com/google/omaha-client/commit/abc12345)
